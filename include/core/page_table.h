...
/* Create a single 4 KiB mapping in the supplied address space. */
bool page_table_map(struct hal_address_space* table, uintptr_t virt, uintptr_t phys, vmm_prot_t prot);

/* Remove a single 4 KiB mapping from the supplied address space. */
bool page_table_unmap(struct hal_address_space* table, uintptr_t virt);

/* Translate a single virtual address into its physical address and reconstructed HAL flags. */
bool page_table_query(const struct hal_address_space* table, uintptr_t virt, uintptr_t* out_phys, uint64_t* out_flags);

/* Atomically swap the physical backing and protection bits for a single mapping. */
