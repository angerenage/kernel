#pragma once

#include <stdint.h>

/*
 * Lock-class metadata shared by spinlock debug builds.
 *
 * Ordering values are monotonically increasing acquisition classes. When
 * CORE_LOCK_DEBUG is enabled, taking locks out of order or without the required
 * irq-save discipline traps immediately.
 */

enum spinlock_order {
	SPINLOCK_ORDER_NONE             = 0u,
	SPINLOCK_ORDER_CPU_TOPOLOGY     = 10u,
	SPINLOCK_ORDER_MUTEX            = 12u,
	SPINLOCK_ORDER_SCHED            = 15u,
	SPINLOCK_ORDER_CLOCK            = 20u,
	SPINLOCK_ORDER_CAPABILITY       = 24u,
	SPINLOCK_ORDER_ID_TABLE         = 25u,
	SPINLOCK_ORDER_PROCESS          = 25u,
	SPINLOCK_ORDER_INTERRUPT        = 26u,
	SPINLOCK_ORDER_SIGNAL           = 27u,
	SPINLOCK_ORDER_USER_UPCALL      = 28u,
	SPINLOCK_ORDER_DMA_CONTROLLER   = 35u,
	SPINLOCK_ORDER_VADDR            = 40u,
	SPINLOCK_ORDER_MEMORY           = 45u,
	SPINLOCK_ORDER_MEMORY_BACKING   = 47u,
	SPINLOCK_ORDER_BACKING_METADATA = 48u,
	SPINLOCK_ORDER_PAGING           = 50u,
	SPINLOCK_ORDER_PMM              = 60u,
	SPINLOCK_ORDER_HEAP             = 70u,
};

/* Extra behavioral requirements that debug builds validate on lock acquisition. */
enum spinlock_flags {
	SPINLOCK_FLAG_NONE            = 0u,
	SPINLOCK_FLAG_IRQSAVE         = 1u << 0,
	SPINLOCK_FLAG_ALLOW_EXCEPTION = 1u << 1,
};
