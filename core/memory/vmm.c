...
bool vmm_init(void) {
	...
	if (!hal_paging_init()) return false;
	if (!vaddr_alloc_init((uintptr_t)KERNEL_HEAP_BASE, heap_pages)) return false;
...
bool vmm_alloc_pages(size_t count, size_t align_pages, uint64_t flags, void** out_virt) {
	...
	if (!vaddr_alloc_reserve(count, align_pages, &virt_base)) return false;
...
bool vmm_free_pages(void* virt, size_t count) {
	...
	for (size_t page = 0; page < count; page++) {
		...
		if (!hal_paging_query(addr, &phys, NULL)) return false;
	}
	...
	for (size_t page = 0; page < count; page++) {
		...
		if (!hal_paging_query(addr, &phys, NULL)) return false;
		if (!hal_paging_unmap(addr)) return false;
		if (!pmm_free_pages(phys & ~(uintptr_t)(PMM_PAGE_SIZE - 1u), 1)) return false;
	}

	return vaddr_alloc_release(virt_base, count);
}
