...
static enum address_transfer_result translate_page(struct address_space* space, uintptr_t addr, uint32_t access,
                                                   uintptr_t* out_phys, size_t* out_chunk) {
	...
	if (!vmm_query(space, (void*)addr, &allocation)) return ADDRESS_TRANSFER_NOT_MAPPED;
	...
	if (!hal_paging_query(address_space_hal(space), page_base, &phys, &flags)) {
		...
		if (!vmm_resolve_page_fault(space, addr)) return ADDRESS_TRANSFER_FAULT_FAILED;
		if (!hal_paging_query(address_space_hal(space), page_base, &phys, &flags)) {
...
enum address_transfer_result address_space_validate_range(struct address_space* space, uintptr_t addr, size_t size,
	...
	while (checked < size) {
		size_t                       chunk;
		enum address_transfer_result result = translate_page(space, addr + checked, access, NULL, &chunk);
...
enum address_transfer_result address_space_copy_between(struct address_space* dst_space, uintptr_t dst_addr,
	...
	while (copied < size) {
		...
		result = translate_page(src_space,
		                        src_addr + copied,
		                        ADDRESS_TRANSFER_READ | ADDRESS_TRANSFER_USER | ADDRESS_TRANSFER_FAULT_IN,
		                        &src_phys,
		                        &src_chunk);
		if (result != ADDRESS_TRANSFER_OK) return result;
		result = translate_page(dst_space,
		                        dst_addr + copied,
		                        ADDRESS_TRANSFER_WRITE | ADDRESS_TRANSFER_USER | ADDRESS_TRANSFER_FAULT_IN,
		                        &dst_phys,
		                        &dst_chunk);
