...
Test(address_transfer, implicit_zero_reads_and_writes_do_not_require_ptes) {
	...
	cr_assert_eq(address_space_copy_from(&space, (uintptr_t)base + PMM_PAGE_SIZE - 8u, bytes, sizeof(bytes)),
	             ADDRESS_TRANSFER_OK);
	...
	cr_assert_eq(address_space_copy_to(&space, (uintptr_t)base + PMM_PAGE_SIZE - 8u, pattern, sizeof(pattern)),
	             ADDRESS_TRANSFER_OK);
...
Test(address_transfer, crosses_mappings_and_enforces_protection) {
	...
	init_test_vmm(arena, sizeof(arena));
	cr_assert(vm_space_create_user(&space));
	uintptr_t base = space.base + 4u * PMM_PAGE_SIZE;
	cr_assert(test_vm_map(&space, 1u, VMM_PROT_READ | VMM_PROT_WRITE, base, 1u, 0u, &first_id, &first));
	cr_assert(test_vm_map(&space, 1u, VMM_PROT_READ | VMM_PROT_WRITE, base + PMM_PAGE_SIZE, 1u, 0u, &second_id, NULL));
	for (size_t i = 0u; i < sizeof(in); i++) in[i] = (uint8_t)(0xa0u + i);
	cr_assert_eq(address_space_copy_to(&space, base + PMM_PAGE_SIZE - 8u, in, sizeof(in)), ADDRESS_TRANSFER_OK);
	cr_assert_eq(address_space_copy_from(&space, base + PMM_PAGE_SIZE - 8u, out, sizeof(out)), ADDRESS_TRANSFER_OK);
	cr_assert_arr_eq(in, out, sizeof(in));
	cr_assert(vm_space_protect(&space, second_id, VMM_PROT_READ));
	cr_assert_eq(address_space_copy_to(&space, base + PMM_PAGE_SIZE, in, 1u), ADDRESS_TRANSFER_ACCESS_DENIED);
