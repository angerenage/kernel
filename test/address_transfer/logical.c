...
Test(address_transfer, implicit_zero_reads_and_writes_do_not_require_ptes) {
	...
	cr_assert(test_vm_map(&space, 2u, MEMORY_ACCESS_READ | MEMORY_ACCESS_WRITE, 0u, 1u, 0u, &mapping, &base));
...
Test(address_transfer, crosses_mappings_and_enforces_protection) {
	...
	cr_assert(test_vm_map(&space, 1u, MEMORY_ACCESS_READ | MEMORY_ACCESS_WRITE, base, 1u, 0u, &first_mapping, &first));
	cr_assert(test_vm_map(&space,
	                      1u,
...
Test(address_transfer, overlapping_copy_has_memmove_semantics) {
	...
	init_test_vmm(arena, sizeof(arena));
	cr_assert(address_space_create_process(&space));
	cr_assert(test_vm_map(&space, 1u, MEMORY_ACCESS_READ | MEMORY_ACCESS_WRITE, 0u, 1u, 0u, &mapping, &base));
