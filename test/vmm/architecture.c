...
Test(address_space, registry_survives_random_insert_and_remove_order) {
	...
	for (size_t i = 0u; i < sizeof(insert_order); i++) {
		...
		cr_assert(
			address_space_resolve_fault(address_space_kernel(), mapping_address(mappings[i]), MAPPING_ACCESS_READ));
		cr_assert_not(address_space_resolve_fault(
			address_space_kernel(), mapping_address(mappings[i]) - 1u, MAPPING_ACCESS_READ));
	}
...
Test(address_space, physical_zero_is_a_valid_backing) {
	...
	cr_assert(memory_create_physical(
		...
		&zero));
	cr_assert(map_memory(address_space_kernel(), zero, 0u, 0u, 0u, 0u, MAPPING_ACCESS_READ, &mapping));
...
Test(address_space, faults_and_prefaults_expose_large_present_runs_to_hal) {
	...
	uintptr_t       first_address  = MM_KERNEL_VMM_BASE + large_size;
	...
	cr_assert(map_memory(
		address_space_kernel(), memory, second_address, large_size, 0u, 0u, MAPPING_ACCESS_READ, &prefault_mapping));
...
Test(address_space, a_fault_maps_at_most_one_supported_leaf) {
	...
	struct memory*  memory;
	struct mapping* mapping;
	uintptr_t       address = MM_KERNEL_VMM_BASE + 4u * leaf_size;
...
Test(address_space, shared_memory_faults_reuse_backing) {
	...
	cr_assert(address_space_resolve_fault(&b, mapping_address(b_mapping), MAPPING_ACCESS_READ));
...
Test(address_space, protect_none_preserves_contents_and_future_access) {
	...
	cr_assert(address_space_resolve_fault(address_space_kernel(), mapping_address(mapping), MAPPING_ACCESS_WRITE));
	cr_assert(memory_write(memory, 0u, &value, sizeof(value)));
	cr_assert(address_space_protect(address_space_kernel(), mapping, MAPPING_ACCESS_READ));
	cr_assert(hal_paging_query(address_space_hal(address_space_kernel()), mapping_address(mapping), &translation));
