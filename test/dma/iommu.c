...
Test(iommu, protect_range_and_hole_rejection) {
	struct hal_iommu_controller_state      controller = {0};
	struct hal_iommu_controller_descriptor descriptor = iommu_descriptor();
	struct hal_iommu_space_state           space      = {0};
	struct hal_iommu_info                  info;
	...
	cr_assert(hal_iommu_controller_init(&controller, &descriptor, &info));
	cr_assert(hal_iommu_space_init(&controller, 1u, &space));
	struct hal_iommu_map_request map_req = {.io_address       = 0x200000u,
	                                        .physical_address = 0x300000u,
	                                        .size             = 0x200000u,
	                                        .access           = HAL_IOMMU_READ | HAL_IOMMU_WRITE};
	cr_assert(hal_iommu_map(&controller, &space, &map_req));
	cr_assert(hal_iommu_protect(&controller, &space, 0x200000u, 0x100000u, HAL_IOMMU_READ));
	cr_assert(!hal_iommu_protect(&controller, &space, 0x500000u, 0x100000u, HAL_IOMMU_READ));
	cr_assert(hal_iommu_unmap(&controller, &space, 0x250000u, 0x10000u));
	cr_assert(!hal_iommu_protect(&controller, &space, 0x200000u, 0x200000u, HAL_IOMMU_READ | HAL_IOMMU_WRITE));
	cr_assert(hal_iommu_protect(&controller, &space, 0x200000u, 0x50000u, HAL_IOMMU_READ)); // from 0x200000 to 0x250000
	cr_assert(
		hal_iommu_protect(&controller, &space, 0x260000u, 0x1a0000u, HAL_IOMMU_READ)); // from 0x260000 to 0x400000
