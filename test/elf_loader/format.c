...
Test(elf_loader_format, rejects_segment_file_ranges_outside_the_module) {
	...
	elf_test_image_init(&image, 1u);
	elf_test_header(&image)->entry = vaddr;
	elf_test_set_load(&image,
	                  0u,
	                  2u * TEST_MAPPING_GRANULE,
	                  vaddr,
	                  TEST_MAPPING_GRANULE + 1u,
	                  TEST_MAPPING_GRANULE + 1u,
	                  ELF_TEST_PF_R | ELF_TEST_PF_X);
	module = elf_test_module(&image);
...
Test(elf_loader_format, rejects_overflowing_virtual_segment_ranges) {
	...
	elf_test_set_load(&image, 0u, TEST_MAPPING_GRANULE, UINT64_MAX - 15u, 8u, 32u, ELF_TEST_PF_R | ELF_TEST_PF_X);
	module = elf_test_module(&image);
...
Test(elf_loader_format, rejects_entry_points_without_executable_mapping) {
	...
	elf_test_image_init(&image, 1u);
	elf_test_header(&image)->entry = vaddr;
	elf_test_set_load(&image, 0u, TEST_MAPPING_GRANULE, vaddr, 16u, 16u, ELF_TEST_PF_R | ELF_TEST_PF_W);
	memset(image.bytes + TEST_MAPPING_GRANULE, 0x24, 16u);
	module = elf_test_module(&image);
