...
static syscall_result_t allocation_map_handler(const struct cap_request* req, struct allocation_state* state,
                                               bool at_explicit) {
	struct allocation_map_at_request request;
	struct address_space*            caller_space;
	struct capability*               allocation_cap;
	uintptr_t                        explicit_address = 0u;
	...
	allocation_cap = cap_lookup(req->cap_id);
	if (allocation_cap == NULL) return syscall_result_error(SYSCALL_STATUS_BAD_ARGUMENT, 0u);
	if (!allocation_mapping_acquire(state)) return syscall_result_error(SYSCALL_STATUS_BAD_ARGUMENT, 0u);

	if (!vmm_alloc_phys(
			caller_space, state->phys_base, state->page_count, state->prot, (void*)explicit_address, &new_id, NULL)) {
		kernel_allocation_mapping_release(state);
		return syscall_result_error(SYSCALL_STATUS_FAILED, 0u);
	}

	mapping_cap = kernel_mapping_create(state, allocation_cap, req->caller, caller_space, new_id, state->kind);
	if (mapping_cap == CAP_ID_INVALID) {
		(void)vmm_free(caller_space, new_id);
		kernel_allocation_mapping_release(state);
		return syscall_result_error(SYSCALL_STATUS_FAILED, 0u);
	}
...
static syscall_result_t allocation_handler(const struct cap_request* req) {
	...
	switch (header.op) {
	case ALLOCATION_OP_MAP:
	case ALLOCATION_OP_MAP_AT:
	...
	switch (header.op) {
	case ALLOCATION_OP_MAP:
		return allocation_map_handler(req, state, false);
	case ALLOCATION_OP_MAP_AT:
		return allocation_map_handler(req, state, true);
	case ALLOCATION_OP_FREE:
...
static bool mapping_owner_is_alive(const struct mapping_state* state) {
	struct process* owner = process_lookup(state->owner);
...
static syscall_result_t mapping_unmap_handler(struct mapping_state* state) {
	if (!mapping_owner_is_alive(state)) return syscall_result_error(SYSCALL_STATUS_BAD_ARGUMENT, 0u);
	...
	if (!vmm_free(state->space, state->region_id)) {
		spinlock_unlock(&state->lock);
		return syscall_result_error(SYSCALL_STATUS_FAILED, 0u);
	}
	state->active = false;
...
static cap_id_t kernel_mapping_create(struct allocation_state* allocation, struct capability* allocation_cap,
	...
	if (allocation == NULL || allocation_cap == NULL || owner == PROCESS_PID_INVALID || space == NULL ||
	    region_id == VMM_ID_INVALID) {
		return CAP_ID_INVALID;
	}
	...
	cap = cap_create(object->cap_object_id, owner, CAP_CALL | CAP_READ | CAP_MAP | CAP_DESTROY, allocation_cap);
	if (cap == NULL) {
		(void)cap_object_destroy(object);
		free(state);
		return CAP_ID_INVALID;
